            ...
            fetch(`../locales/${lang}.json`)
                ...
                .then(translations => {
                    ...
                    if (window.location.pathname.endsWith('index.html')) {
                        generateSections(data.sections, translations)
                        .then(() => {
                            setupIntersectionObserver(); 
                            initializeI18n(observer); // Inicializa la internacionalización// Agregar un retraso de 0.5 segundos antes de iniciar el observer
                        });// Configurar Intersection Observer  
                    } else {
                        console.log('Se ejecuto el else');
                        handleInitialHash();
                        initializeI18n(); // Inicializa la internacionalización
...
function setupIntersectionObserver() {
    console.log('Se inicio el observer')
    const options = {
